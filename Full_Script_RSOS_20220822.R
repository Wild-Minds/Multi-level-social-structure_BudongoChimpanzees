...
library(dplyr)
#create data frame with Waibira core edgelist and add column comparing dyad edge weight to chance edge weight 
WA_edges<-as.data.frame(cbind( get.edgelist(netWA) , round( E(netWA)$weight, 3 )))
colnames(WA_edges)<-c("ID_A", "ID_B", "weight")
WA_edges$weight<-as.numeric(WA_edges$weight)
WA_edges$diff<-WA_edges$weight-(mean(randomEW))
head(WA_edges)

#create data frame with Waibira periphery edgelist and add column comparing dyad edge weight to chance edge weight 
WB_edges<-as.data.frame(cbind( get.edgelist(netWB) , round( E(netWB)$weight, 3 )))
colnames(WB_edges)<-c("ID_A", "ID_B", "weight")
WB_edges$weight<-as.numeric(WB_edges$weight)
WB_edges$diff<-WB_edges$weight-(mean(randomEW))
head(WB_edges)

#create data frame with full Waibira edge list and column comparing dyad edge weight to chance edge weight
W_edges<-as.data.frame(cbind( get.edgelist(netW) , round( E(netW)$weight, 3 )))
colnames(W_edges)<-c("ID_A", "ID_B", "weight")
W_edges$weight<-as.numeric(W_edges$weight)
W_edges$diff<-W_edges$weight-(mean(randomEW))
head(W_edges)

#subset the full Waibir edgelist to dataframe including only core-periphery connections
#create list of edges between clusters 
tm_edges<-rbind(WA_edges,WB_edges,W_edges)
WAB_edges<-tm_edges %>% 
  count(ID_A, ID_B, weight, diff) %>% 
  filter(n == 1)
head(WAB_edges)

#normalise difference between dyad and chance edge weight
WA_edges$norm<-((WA_edges$diff)-min(W_edges$diff))/(max(W_edges$diff)-min(W_edges$diff))
WB_edges$norm<-((WB_edges$diff)-min(W_edges$diff))/(max(W_edges$diff)-min(W_edges$diff))
WAB_edges$norm<-((WAB_edges$diff)-min(W_edges$diff))/(max(W_edges$diff)-min(W_edges$diff))

#get mean normalised different for core-core, periphery-periphery and core-periphery edges
Pcc<-mean(WA_edges$norm)
Ppp<-mean(WB_edges$norm)
Pcp<-mean(WAB_edges$norm)
